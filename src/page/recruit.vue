<template>
  <div>
    ...
    <ul class="newslist" v-load-more="loaderMore">
      <li v-for="(item,index) in recruitCont" :key="index">
        <router-link :to="{path:'/recruitDetail',query:{id: item.id}}">
          <h4>
            <p class="t"><span class="hot" v-if="item.listTop">[顶]</span>{{item.title}}</p>
            <p class="time"><span>{{item.time}}</span><span>{{item.address}}</span><span>{{item.people}}人</span><span>{{item.checkInfo}}</span></p>
          </h4>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      async initData (){
        //获取数据
        let res = await recruit(this.contId,this.page);
        this.recruitCont = [...res.data];
      ...
      async loaderMore(){
        ...
        this.page += 1;
        let res = await recruit(this.contId,this.page);
        this.recruitCont = ([...this.recruitCont,...res.data]);
