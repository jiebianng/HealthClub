   ...
   <div>
     <div class="sb_nav">
       <h3 class="title">
         ...
         <label>{{alertMupTitle}}</label>
       </h3>
     </div>
     <div class="sidebar" :class='{active : openSideIf}'>
       <h3 class="title" @click="closeSidebar">{{alertMupTitle}}</h3>
       <dl class="list-none" @click="closeSidebar" v-for="(item,index) in menuCont" :key="index">
         <dt><router-link :to="{path: alertMupType, query:{swiperSlide:1,id:item.id}}" :title="item.title" class="zm"><span>{{item.title}}</span></router-link></dt>
...
<script>
    ...
    export default {
        ...
        props:['alertMupType','alertMupTitle'],
        methods: {
          //初始化时获取基本数据
